...
<div id="order_div" class='col-lg-12 row'>
	<?
		$sql = "select a.* ,c.user_nick,b.store_name from `order` a inner join store b on a.store_id = b.store_id
		inner join user c on a.order_user_id = c.user_id
		where b.user_id = ".$_SESSION["user_id"]." AND (a.pay_state >= 1 AND a.item_state >= 4) order by a.creat_date desc";
	...
	?>
			<table class="table orderTable">
				<tr><th>订单编号</th><th>开始/结束时间</th><th>買家名稱</th><th>订单金额</th><th>订单状态</th></tr>
				<tr>
	...
	function show_order_info(order_id){
		if(Dd('order_info_div'+order_id).style.display=='none'){
	      ...
	      success: function(myjson){
	      	...
	      	str += "<tr class='greenTitle'><th colspan='2'>商品清单</th><th>价格</th><th>数量</th><th>总价</th></tr>";
	      	...
	      	str += "<tr><td class='greenTitle'>订单备注</td><td colspan='4'>"+myjson.rec_note+"</td></tr></table>";
	      	str += "</div><div class='col-lg-4'><table class='table orderDetail_table'><tr><td class='greenTitle'>商品金额</td><td>";
	      	if(myjson.total_item_price != myjson.edit_price)
	      		str += myjson.edit_price+"（"+myjson.total_item_price+"）";
	      	else
	      		str += myjson.edit_price;
	      	str += "</td></tr><tr><td class='greenTitle'>运费</td><td>"+myjson.order_shipment+"</td></tr><tr><td class='greenTitle'>订单总额</td><td><h3>"+myjson.total_price+"</h3></td></tr></table></div>";
