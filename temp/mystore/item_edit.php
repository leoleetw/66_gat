...
?>
<div id="item_div" class='col-lg-10'>
	<font style="color:red">编辑完毕後，商品需要重新上架</font>
	<form id="item_form" action="ajax/item.php" method="post" enctype="multipart/form-data">
		...
		<div class="row">
			...
			<div class='col-lg-10'><input type="text" class="form-control" id="item_name" name="item_name" value="<? echo $row_item["item_name"]; ?>" ></div>
		</div>
		<div class="row">
		...
		<div class="row">
			...
			<div class='col-lg-10'><input type="text" class="form-control" id="item_stock" name="item_stock" value="<? echo $row_item["item_stock"]; ?>" placeholder='999内' onchange="if(isNaN(this.value)){alert('只能填写数字');this.value='';this.focus();};"></div>
		</div>
		<div class="row">
			<div class='col-lg-2' style="text-align:right;"><font style="color:red">＊</font><font>商品价格︰</font></div>
			<div class='col-lg-10'><input type="text" class="form-control" id="item_price" name="item_price" value="<? echo $row_item["item_price"]; ?>" onchange="if(isNaN(this.value)){alert('只能填写数字');this.value='';this.focus();};"></div>
		</div>
		<div class="row">
			<div class='col-lg-2' style="text-align:right;"><font style="color:red">＊</font><font>商品照片︰<br>(上限5张<br>第一张为主图片)</font></div>
			<div class="col-lg-10">
				<?
					$photo = explode("|",$row_item["item_photo"]);
					for($i = 0; $i < count($photo); ++$i){
				?>
						<img id="item_img<? echo ($i+1); ?>" src="update/item_s/<? echo $photo[$i]; ?>" style="width:100px;" onclick="$('#item_photo<? echo ($i+1); ?>').click();">
		...
		<div class="row">
			...
			<div class='col-lg-10'>
				<table class="table table-bordered">
					<tr>
						<td><font>尺寸</font></td><td><input type="text" class="form-control" id="item_size" name="item_size" value="<? echo $row_item["item_size"]; ?>" placeholder='20字内描述'></td>
						<td><font>产地</font></td><td><input type="text" class="form-control" id="item_origin" name="item_origin" value="<? echo $row_item["item_orgian"]; ?>" placeholder='20字内描述'></td>
					</tr>
					<tr>
						<td><font>重量</font></td><td><input type="text" class="form-control" id="item_weigth" name="item_weigth" value="<? echo $row_item["item_weight"]; ?>" placeholder='20字内描述'></td>
						<td><font>包装尺寸</font></td><td><input type="text" class="form-control" id="package_size" name="package_size" value="<? echo $row_item["package_size"]; ?>" placeholder='20字内描述'></td>
					</tr>
					<tr><td><font>注意事项</font></td><td colspan='3'><textarea class="form-control" id="item_note" name="item_note" placeholder='50字内描述'><? echo $row_item["item_note"]; ?></textarea></td></tr>
				</table>
			</div>
		</div>
		<div class="row">
