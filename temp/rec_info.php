				...
				<div class="recProduct">
					...
					?>
						<div class="col-xs-6 col-lg-3">
							<div class="merchandise_wrapper recProduct_wrapper">
...
<?
		...
		if($count != 0){
			$row = mysqli_fetch_array($result);
?>
			<button type='button' id='next_btn' class='btn btn-primary' onclick="location.href='rec_info.php?rec_id=<? echo $row["rec_id"];?>'">上一则</button>
<?
		}
		...
		if($count != 0){
			$row = mysqli_fetch_array($result);
?>
			<button type='button' id='last_btn' class='btn btn-primary' onclick="location.href='rec_info.php?rec_id=<? echo $row["rec_id"];?>'">下一则</button>
<?
		}
?>
		</div>
