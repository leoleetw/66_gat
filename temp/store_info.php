...
<div class="col-lg-9">
  <?
    $sql = "select * from store where store_id = '".$_GET["s_id"]."'";
    ...
    <div class="col-lg-12">
      <div class="seller_info">
        ...
        <div class="store_infoWrapper">
          <h4><font></font><? echo $row["store_name"]; ?></h4>
          <p><font></font><? echo $row["store_introduce"]; ?></p>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="col-lg-12">
    <div class="row">
      <div class="newStore">
        ...
        <?
          $sql_newitem = "select * from item where store_id =".$_GET["s_id"]." and item_state = 1 order by shelves_date DESC limit 0,3";
  ...
  <div id='seq_div' class="sorting02">排序︰
    <?
      $get_value = "&s_id=".$_GET["s_id"];
      if($_GET["seq"]=="")
        $seq = "name";
      else
        $seq = $_GET["seq"];
    ?>
    <button class="btn btn-<? if($seq=="name"){ echo "danger";}else{ echo "default";} ?>" onclick="location.href='store_info.php?seq=name<? echo $get_value;?>'" >商品名称</button>
    <button class="btn btn-<? if($seq=="exp"){ echo "danger";}else{ echo "default";} ?>" onclick="location.href='store_info.php?seq=exp<? echo $get_value;?>'" >价钱高低</button>
    <button class="btn btn-<? if($seq=="new"){ echo "danger";}else{ echo "default";} ?>" onclick="location.href='store_info.php?seq=new<? echo $get_value;?>'" >最新上架</button>
  </div>
  </div>
  <div class="numberButton">
    ...
    <?
      ...
      where a.item_state = 1 and  a.store_id =".$_GET["s_id"]."";
      $sql_item .= " order by ";
      if($seq == 'name')
    ...
    ?>
        <li <? if($now_page == 1 ){echo "style='display:none;'";} ?>><a class='be4_numberButton' href="store_info.php?now_page=1<? echo $get_value; ?>">&laquo;</a></li>
        <li <? if($now_page == 1 ){echo "style='display:none;'";} ?>><a class='be4_numberButton' href="store_info.php?now_page=<? echo ($now_page-1).$get_value; ?>">&#8249;</a></li>
      <?
        ...
        if($page_error != 0)
          ...
          for( $n = 0; $n < 5 && $page <= $page_count; ++$n , ++$page){
            ...
            echo "<li><a href='store_info.php?now_page=".$page.$get_value."' ";
            if($page == $now_page)
      ...
      ?>
      <li <? if($page_count <= 1 || $page_count == $now_page){echo "style='display:none;'";} ?>><a class='be4_numberButton' href="store_info.php?now_page=<? echo ($now_page+1).$get_value; ?>">&#8250;</a></li>
      <li <? if($page_count == $now_page){echo "style='display:none;'";} ?>><a class='be4_numberButton' href="store_info.php?now_page=<? echo $page_count.$get_value; ?>">&raquo;</a></li>
    </ul>
  </div>
  <div class="row">
    <div id="item_div" class="storeProduct">
      <?
