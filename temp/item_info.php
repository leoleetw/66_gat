...
<div class="col-lg-9">
	...
	<div class="row">
		...
		<div class="col-lg-4">
			...
			<div class="seller_info">
				<p><font>商品名︰</font><? echo $row["item_name"]; ?></p>
				<p><font>商品价格︰</font><? echo $row["item_price"]; ?></p>
				<p><font>商品库存︰</font><? echo $row["item_stock"]; ?></p>
				<p><font>商品种类︰</font><? echo $row["cate_name"]; ?></p>
			</div>
			<button id="add_cart_btn" type="button" class="btn btn-default" onclick="addtocart(<? echo $row["item_id"]; ?>);this.disabled='disabled';"> 放入购物车</button>
			<button type="button" class="btn btn-default" onclick="addtocollect(<? echo $row["item_id"].",".$row["store_id"]; ?>);this.disabled='disabled';" <? if($_SESSION["user_id"]=="" || $row["collect"]!=null){ echo "disabled";} ?>> 加入收藏</button>
		</div>
	</div>
	<div class="row">
		<div class="col-lg-12">
