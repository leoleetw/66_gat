...
<div id="order_div" class='col-lg-12'>
	...
	<?
		...
		where a.order_user_id = ".$_SESSION["user_id"]." AND a.order_state = 0 AND( a.pay_state < 2 OR a.item_state < 4) order by a.creat_date desc";
	...
	?>
			<table class="table orderTable">
				<tr><th>卖家确认</th><th>订单编号</th><th>开始时间</th><th>卖家名稱</th><th>订单金额</th><th>买家付款状态</th><th>订单状态</th></tr>
				<tr>
					<td>
						<?
					...
					<td><a onclick="show_order_info('<? echo $row["order_id"]; ?>');"><? echo $row["order_id"]; ?></a></td>
	...
	function show_order_info(order_id){
		if(Dd('order_info_div'+order_id).style.display=='none'){
	      ...
	      success: function(myjson){
	      	...
	      	str += "</td></tr><tr><td class='blueTitle'>运费</td><td>"+myjson.order_shipment+"</td></tr><tr><td class='blueTitle'>订单金额</td><td>"+myjson.total_price+"</td></tr><tr><td colspan='2' class='waitCheck'>";
	      	if(myjson.order_check == 0)
	      		str += "<a>等候卖家确认</a>";
	      	else if(myjson.order_check == 1 && myjson.pay_state == 0)
	      		str += "<a onclick='payinfo("+order_id+")'>取得付款资讯</a>";
	      	else if(myjson.order_check == 1 && myjson.pay_state > 0)
	      		str += "<a>此订单已付款<a>";
	      	else
	      		str += "<a>看到此讯息请通知官方</a>";
	      	str += "</td></tr></table></div>"
	      	$('#order_info_div'+order_id).html(str);
	      	$('#order_div>div>div').stop(true,true).slideUp('middle');
	      	$('#order_info_div'+order_id).stop(true,true).slideDown('middle');
